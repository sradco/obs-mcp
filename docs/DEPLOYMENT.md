...
- `--insecure` skips TLS verification. The bearer token is forwarded only to loopback HTTPS (`localhost` / `127.0.0.1` / `::1`), for port-forwards. OpenShift route hostnames need a trusted CA instead of `--insecure`.
